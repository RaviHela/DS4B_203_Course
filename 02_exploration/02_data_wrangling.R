...
subscribers_weekly_tbl <- mailchimp_users_tbl %>%
  summarise_by_time(optin_time, .by = "week", optins = n())
...
# With floor and ceiling and round
transactions_tbl %>%
  summarise_by_time(purchased_at,
                    .by = "1 month", 
                    revenue = sum(revenue),
...
transactions_tbl %>%
  ...
  mutate_by_time(.date_var = purchased_at, .by = "week", revenue_spread = sum(revenue, na.rm = T)/7 )
...
transactions_tbl %>%
  mutate_by_time(
    .by = "month", 
    revenue_mean = mean(revenue),
    revenue_mdian = median(revenue), 
    revenue_max = max(revenue),
    revenue_min = min(revenue)
  ) %>%
  pivot_longer(contains("revenue")) %>%
  plot_time_series(purchased_at, value, .smooth = FALSE, .color_var = name)
...
subscribers_daily_padded_tbl <-  subscribers_daily_tbl %>%
  pad_by_time(optin_time, .pad_value = 0, .start_date = "2018")
...
log_standardized_subscribers_joined_tbl %>%
  plot_acf_diagnostics(optin_time, optins, .ccf_vars = pageViews:sessions, .show_ccf_vars_only = TRUE)
...
future_tbl <- google_analytics_summary_daily_tbl %>%
  future_frame(.length_out = "2 month")
...
google_analytics_summary_daily_tbl %>%
  ...
  bind_rows(future_tbl %>% mutate(pageViews = predictions_vec, type = "prediction")) %>%
